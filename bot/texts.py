...
GROUP_RULES_SHORT_TEXT = (
    ...
    "📝 Напиши «Рассказ о себе» в теме группы\n"
    "👤 Укажи имя и фото (если ещё нет в Telegram)\n"
    "⏳ Срок — 7 дней с момента вступления"
)


def get_group_rules_text() -> str:
    """Полные правила с контактом владельца из OWNER_CONTACT / OWNER_ID."""
    from bot.config import OWNER_CONTACT, OWNER_ID
    from bot.utils.helpers import build_owner_contact_html

    owner_contact = build_owner_contact_html(OWNER_CONTACT or None, OWNER_ID, label="капитану")
    return (
        "<b>📜 Полные правила группы</b>\n\n"
        "<b>🎯 Цель</b>\n"
        "Находить друзей и встречаться!\n\n"
        "<b>✅ Можно</b>\n"
        "💡 Делиться идеями, мыслями, историями\n"
        "🎉 Обсуждать всё\n"
        "👋 Знакомиться\n\n"
        "<b>🚫 Нельзя</b>\n"
        "Политика, рознь, дискриминация, срачи, оскорбления, троллинг, буллинг, чрезмерный флуд\n"
        "Фейки, личка без разрешения, реклама без разрешения\n"
        "Нарушение законов РФ\n\n"
        "<b>⚠️ Наказания</b>\n"
        "Предупреждение → мут → бан\n"
        f"Разбан: напиши {owner_contact}\n"
        "Повторные нарушители могут не восстанавливаться.\n\n"
        "<b>📝 Обязательно для новичков</b>\n"
        "1️⃣ Рассказ о себе в теме «Рассказ о себе»\n"
        "2️⃣ Имя и фото в профиле Telegram (если ещё нет)\n"
        "3️⃣ Кратко — что интересно в группе\n"
        "4️⃣ Срок: 7 дней с момента вступления\n\n"
        "📝 Администрация вправе менять правила."
    )


# Совместимость со старыми импортами: GROUP_RULES_TEXT — вызов get_group_rules_text().
# Используйте get_group_rules_text() в новом коде.
...
async def format_event_message(
    ...
    *,
    ...
    if gcal_link:
        link_parts.append(f'<a href="{gcal_link}">📅 Google Календарь</a>')
    if ycal_link:
...
def format_attendance_prompt_text(event: Dict, hours_before: int) -> str:
    ...
    return (
        f"🗓 <b>Подтверди участие</b>\n\n"
        f"Через ~{hours_before} ч стартует:\n"
        f"📌 <b>{title}</b>\n"
        f"📅 {date_str}\n"
        f"📍 {location}\n\n"
        f"Ты всё ещё идёшь? Нажми кнопку ниже — так организатор видит реальную явку."
    )
