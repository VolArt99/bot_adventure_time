...
COMMAND_SPECS: tuple[CommandSpec, ...] = (
    ...
    CommandSpec("create_event", "create_event", "Запустить мастер создания мероприятия (дата/время отдельно, ссылка, длительность вроде «1 ч 30 мин»).", "events", CommandKind.ACTION),
    ...
    CommandSpec("edit_event", "edit_event", "Редактировать поля мероприятия (в т.ч. ссылку и длительность). Пример: <code>/edit_event 42</code>", "events", CommandKind.HELP, "/edit_event &lt;event_id&gt;"),
    CommandSpec("cancel_event", "cancel_event", "Устарело: отмена через кнопку «Удалить» в карточке мероприятия (ЛС).", "admin", CommandKind.HELP),
    ...
    CommandSpec("birthday", "birthday", "Показать сохранённый день рождения.", "community", CommandKind.ACTION),
    CommandSpec("set_birthday", "set_birthday", "Указать день рождения (ДД.ММ). Пример: <code>/set_birthday 15.07</code>", "community", CommandKind.HELP, "/set_birthday &lt;ДД.ММ&gt;"),
    CommandSpec("clear_birthday", "clear_birthday", "Удалить день рождения из профиля.", "community", CommandKind.ACTION),
    ...
    CommandSpec(
        "refresh_event_cards",
        "refresh_event_cards",
        "Пакетно обновить текст и кнопки карточек активных/будущих мероприятий (после деплоя). Также: меню Админ → Контент.",
        "admin",
        CommandKind.ACTION,
    ),
