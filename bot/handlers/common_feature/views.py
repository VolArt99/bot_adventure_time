...
SECTION_TONES = {
    ...
    "admin_content": {
        ...
        "focus": [
            ...
            "• обновить карточки после деплоя;",
            "• случайные пары 1:1.",
        ],
...
INLINE_BUTTON_HELP = [
    "✅ В путь / ⏳ В резерве / ❌ Снять запись — обновляют статус участия в карточке мероприятия.",
    "Можно переключаться: из резерва «В путь!» → основной состав (если есть место); из основного «В резерв» → резерв.",
    "В ЛС кнопки показывают твой текущий статус (в резерв / снять запись, в путь / снять резерв).",
    "👥 Гости — указать число гостей вне группы (бот спросит в ЛС). В карточке: @user + 2 гостя. Нужно быть записанным.",
...
COMMAND_ACTIONS.update({
    ...
    "create_event": ("/create_event", "Запустить мастер создания мероприятия (дата/время отдельно, ссылка, длительность вроде «1 ч 30 мин»)."),
    ...
    "edit_event": ("/edit_event &lt;event_id&gt;", "Редактировать поля мероприятия, включая ссылку и длительность (создатель, ответственный или админ). Пример: <code>/edit_event 42</code>"),
    "cancel_event": ("/cancel_event", "Устарело: отмена через кнопку «Удалить» в карточке мероприятия (ЛС)."),
    ...
    "refresh_event_cards": (
        "/refresh_event_cards",
        "Пакетно обновить текст и кнопки карточек активных/будущих мероприятий (после деплоя). Также: меню Админ → Контент → «Обновить карточки».",
    ),
