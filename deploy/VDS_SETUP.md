...
> Полный список пользовательских команд бот публикует сам при старте (`bot/main.py` → `setup_bot_commands` из `bot/commands.py`). Справка `/help` (команды + описание кнопок вроде **👥 Гости**) и разделы `/menu` синхронизированы с тем же реестром. Ручной `/setcommands` в BotFather — опциональный краткий набор.
...
| `/help` | все команды участника (+ админ-блок для админов), включая описание кнопок карточки (`👥 Гости` и др.) |
| `/refresh_event_cards` (админ) | после деплоя — пакетно обновить текст/кнопки будущих карточек |
...
- [ ] После обновления с новыми кнопками карточек: `/refresh_event_cards` (или Админ → Контент → «Обновить карточки»)
