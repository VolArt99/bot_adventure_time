...
Telegram-бот для приватного сообщества: мероприятия, участие, напоминания, дайджесты, случайные встречи 1:1, онбординг новых участников и разделение чеков («скинуться»).
...
│   ├── split_bill.py
│   ├── subscriptions.py
│   ├── notification_settings.py   # режим ЛС-уведомлений: all / mine / off
│   ├── usage.py
...
- `participation.py` — кнопки участия (в путь / резерв / снять запись, переключение резерв↔основной, карпулинг, «ищу попутку», подтверждение участия за 24ч).
...
### Базовые
- `/start` — вход/онбординг; для одобренного участника — приветствие, чеклист «что дальше» и кнопки **📣 Афиша** / **🏠 Открыть меню**.
- `/help` — подробная справка без прикреплённого меню.
- `/menu` — **Центр приключений** с разделами 🎉 События, 🧾 Скинуться, 🔔 Уведомления, 🤝 Комьюнити (+ 🔴 Админ для админов).
...
- `/create_event` — пошаговое создание события:
  - дата и время начала **отдельно** (есть быстрые кнопки «сегодня вечером» и т.п.);
  - разовое событие или **период действия** (дата/время конца отдельно);
  - длительность: `1 ч 30 мин`, `90 мин` или число с уточнением «часы/минуты»;
  - опциональная **ссылка** (сайт, чат, регистрация);
  - ответственный, цена, лимит, карпулинг, тема, категории, превью перед публикацией.
...
- `<code>/edit_event &lt;event_id&gt;</code>` — редактирование полей карточки, включая ссылку и длительность (создатель, ответственный или админ).
...
- `/cancel_event` — устарело (подсказка); удаление — кнопка в карточке в ЛС.
### Дайджесты
...
### Random 1:1 (случайные встречи)
- `/random_optin` / `/random_optout` — включить или выключить участие (в `/menu` → Комьюнити — одна toggle-кнопка с текущим статусом).
...
### Сервисные/админские
- `/roles`, `/usage_stats`, `/debug_info`, `/list_topics`, `/update_topic_names`, `/admin_report`, `/pending_intro`, `/send_events_list`, `/refresh_event_cards`, `/member_reengage`, `/sync_members`, `/reset_user_limit <user_id|@username>`, `/cancel_event` (устарело).
- `/refresh_event_cards` — пакетно обновляет текст и кнопки карточек всех **активных будущих** мероприятий (и с незакрытым периодом). После деплоя новых кнопок (например **👥 Гости**) — запустить один раз.

### UX и визуальное оформление

#### Голос и тон
- Единый регистр **«ты»** во всех пользовательских текстах (онбординг, меню, участие, ошибки, карпулинг).
- Тон — приключенческий (Adventure Time community), без копирования персонажей мультсериала.
- Централизация: `bot/utils/design.py` (`BRAND_VOICE`, `SEASON_COPY`, `WIZARD_PROMPTS`).

#### Главное меню (`/menu`)
- Компактная шапка: **«Центр приключений»** + сезонный слоган «Куда отправимся сегодня?» (без длинного списка описаний кнопок).
- **🎉 События** → 👀 Смотреть / ➕ Создать / 🛠 Управление.
- **🧾 Скинуться** — split bill и ручные команды добавления/удаления из чека.
- **🔔 Уведомления** — подписки на категории, **режим ЛС-уведомлений** (`all` / `mine` / `off`), персональный дайджест.
- **🤝 Комьюнити** — случайные встречи 1:1 (toggle), статистика, топ, день рождения.
- **🔴 Админ** — три подраздела: 👥 Люди / 📣 Контент / ⚙️ Система.
  - В **Контент**: отчёт, афиша, **🔄 Обновить карточки** (`/refresh_event_cards`), случайные пары.
- Навигация: в подменю одна кнопка **↩️ Назад** (без дубля «Главное меню + К событиям»).
- Кнопки-подсказки по командам помечены **⌨️ /command** и показывают карточку «Подсказка · команда».

#### Участие и карточки
- Кнопки в **групповой** карточке: `✅ В путь!` / `⏳ В резерве` / `❌ Снять запись` / `👥 Гости`; кнопка **🤖 Открыть в ЛС** для незнакомых с ботом.
- `👥 Гости` — бот пишет в ЛС «сколько гостей?»; в списке «Идут»: `@user + 2 гостя`. Гости учитываются в лимите мест. `0` — убрать гостей. Нужно быть записанным.
- В **ЛС** (через «Мои встречи») — персонализированные:
  - основной состав: `⏳ В резерв` / `❌ Снять запись` / `👥 Гости`;
  - резерв: `✅ В путь!` / `❌ Снять резерв`.
- Переключение статуса одним нажатием: из резерва «В путь!» → основной состав (если есть место); из основного «В резерв» / «В резерве» → резерв (освободившееся место уходит первому в очереди).
- Резерв: toast **«В резерве · ты №N…»**; при продвижении из резерва — брендовое ЛС.
- Внизу групповой карточки — короткий CTA: «Запишись: В путь · Резерв · Снять запись».
- Карпулинг в ЛС: брендовые формулировки («Сколько мест возьмёшь», «К кому присоединиться»).
- Удаление мероприятия — только в ЛС организатора, с подтверждением.

#### Уведомления
- **Режим уведомлений** (`users.notification_settings`):
  - `all` — подписки на категории + личные (явка, резерв, чеки, напоминания);
  - `mine` — только личные;
  - `off` — без пушей в ЛС (ответы бота в диалоге не блокируются).
- Push подписчикам категории при публикации события — с учётом режима и тихих часов `QUIET_HOURS_*`.
- Ночные ЛС ставятся в `pending_notifications` и уходят после тихих часов.

#### Онбординг и правила
- Краткие правила на шаге 2 + подсказка «Полный текст — по кнопке ниже».
- Полные правила структурированы (Цель → Можно → Нельзя → Наказания → Обязательно), без жёстких формулировок.
- После одобрения: чеклист с приоритетом «Рассказ о себе» + кнопки **📣 Афиша** и **🏠 Открыть меню**.
- Неодобренный участник в группе при нажатии на кнопки карточки: **«Напиши боту в ЛС /start…»**.

#### Прочее UX
- За 24 ч до события (`ATTENDANCE_CONFIRM_HOURS`) — ЛС «Ты всё ещё идёшь?» с кнопками confirm/decline.
- Организатор: сводка явки в «Мои встречи» и кнопка «✅ Явка»; empty-alert при 0 участниках.
- 1-го числа месяца owner получает monthly re-engage отчёт.
- Карточка мероприятия: заголовок → статусы → шкала мест `🎟 ████░░░░ 4/10` (с учётом гостей); опциональная ссылка события; ссылки на карты и **Google Календарь** / Я.Календарь.
- Дайджест: «Афиша приключений» + подсказка для iPhone; пустой период — «Тишина в Ланде Ооо…».
- Split-bill: шкала сбора + «осталось собрать X ₽»; отмена мастера — отдельное сообщение «Чек не создан».
- Мастер событий: дата и время отдельно; быстрые даты; длительность `1 ч 30 мин` / уточнение часов-минут; опциональная ссылка; превью с `🗺️ Публикуем!`.
- Подписки: автосохранение. День рождения: одна кнопка в меню → подменю с `/set_birthday` и `/clear_birthday`.
- Статусы карточки: 🔥 скоро, ✅ набор открыт, ⏳ резерв, 🚫 мест нет.
- Дневной лимит команд: дружелюбные сообщения («завтра снова в путь, или напиши капитану»).
...
- Кнопки `menu_cmd_*` в `/menu` показывают **подсказку** (⌨️ + синтаксис и описание), но не выполняют команду; админские подсказки скрыты от обычных участников (`can_view_command_hint`). Частые действия — кнопками `menu_action_*` (афиша, мои встречи, split bill и т.д.).
...
- **Участие (2026-07):** кнопка отказа переименована в `❌ Снять запись` (вместо «В другой раз»); из резерва «В путь!» переводит в основной состав при наличии места; из основного «В резерв» — в резерв с автопродвижением очереди.
- **Мастер событий и гости (2026-07):** дата/время начала раздельно; для периода — дата/время конца раздельно; опциональная ссылка в карточке; длительность `1 ч 30 мин` / `90 мин` / число с уточнением часы·минуты; кнопка **👥 Гости** на карточке → ЛС → `@user + N гостя` (учитывается в лимите мест); админ `/refresh_event_cards` — пакетное обновление старых карточек.
- **UX/UI polish (2026-07):** единый голос «ты»; компактное меню «Центр приключений»; раздел **🧾 Скинуться**; режим ЛС-уведомлений (`all` / `mine` / `off`) в UI и фильтрация push; админ-меню в 3 группы (Люди / Контент / Система); toggle случайных встреч 1:1; резерв с позицией «ты №N»; CTA в групповых карточках; карпулинг и онбординг в брендовом тоне; кнопки ⌨️ для подсказок-команд; смягчённые полные правила; кнопка **🤖 Открыть в ЛС** в группе.
...
|-------------------|------------|
| `BRAND_VOICE` | Фразы бота: меню, онбординг, участие, карпулинг, лимиты, уведомления |
...
Карточка мероприятия: layout **Hero + детали** (шапка → описание → «О мероприятии» → участники), погода в день события (`WEATHER_API_KEY`), CTA «Запишись: В путь · Резерв · Снять запись» в группе.

Split-bill: блоки «Сбор средств» и «Чек-лист оплат»; отмена мастера — `split_bill_cancel` в `BRAND_VOICE`.
