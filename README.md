...
#### Участие и карточки
- Кнопки в **групповой** карточке: `✅ В путь!` / `⏳ В резерве` / `❌ В другой раз` / `👥 Гости`; кнопка **🤖 Открыть в ЛС** для незнакомых с ботом.
...
- В **ЛС** (через «Мои встречи») — персонализированные (`❌ Снять запись`, `❌ Снять резерв`, `👥 Гости`).
- Резерв: toast **«В резерве · ты №N…»**; при продвижении из резерва — брендовое ЛС.
- Внизу групповой карточки — короткий CTA: «Запишись: В путь · Резерв · В другой раз».
...
Карточка мероприятия: layout **Hero + детали** (шапка → описание → «О мероприятии» → участники), погода в день события (`WEATHER_API_KEY`), CTA «Запишись: В путь · Резерв · В другой раз» в группе.
