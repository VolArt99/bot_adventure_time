...
#### Участие и карточки
- Кнопки в **групповой** карточке: `✅ В путь!` / `⏳ В резерве` / `❌ В другой раз`; кнопка **🤖 Открыть в ЛС** для незнакомых с ботом.
- В **ЛС** (через «Мои встречи») — персонализированные (`❌ Снять запись`, `❌ Снять резерв`).
...
- Карточка мероприятия: заголовок → статусы → шкала мест `🎟 ████░░░░ 4/10`; ссылки на карты и **Google Календарь** / Я.Календарь.
...
- Мастер событий: быстрые даты, превью с `🗺️ Публикуем!`.
